...
class App extends Component {
  ...
  componentWillUnmount() {
    const {timeElpasedInSeconds} = this.state
    if (timeElpasedInSeconds === 0) {
      clearInterval(this.timerID)
      this.setState(prevState => ({scoreCard: !prevState.scoreCard}))
    }
  }

  onChangeTab = tabValue => {
    this.setState({isActiveTab: tabValue})
  }

  onGetTheActiveTabItem = () => {
    const {isActiveTab} = this.state
    const filterdItems = imagesList.filter(
      eachImg => eachImg.category === isActiveTab,
    )
    return filterdItems
  }
  ...
  onClickofTumbnail = id => {
    ...
    if (id === randomImgId) {
      const imgUrl = imagesList[Math.ceil(Math.random() * imagesList.length)]
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImgId: imgUrl.id,
        randomImgUrl: imgUrl.imageUrl,
      }))
    } else {
      this.setState(prevState => ({scoreCard: !prevState.scoreCard}))
      clearInterval(this.timerID)
  ...
  tick = () => {
    ...
    if (timeElpasedInSeconds === 0) {
      ...
      this.setState(prevState => ({
        timeElpasedInSeconds: prevState.timeElpasedInSeconds - 1,
      }))
    }
  }
  ...
  renderSeconds = () => {
    const {timeElpasedInSeconds} = this.state
    const seconds = Math.floor(timeElpasedInSeconds % 60)
  ...
  render() {
    const {
      ...
      randomImgUrl,
      scoreCard,
      timeElpasedInSeconds,
    } = this.state
    const filterdList = this.onGetTheActiveTabItem()
    return (
      <div className="bg-container">
        <Header
          score={score}
          timeElpasedInSeconds={timeElpasedInSeconds}
          renderSeconds={this.renderSeconds()}
        />
        <div className="items-container">
          {scoreCard ? (
            <div>
              <img src={randomImgUrl} className="tumbnail-img" alt="match" />
              ...
              <ul className="project-item-list">
                {filterdList.map(eachItem => (
                  <ProjectItem
                    details={eachItem}
                    key={eachItem.id}
                    onClickofTumbnail={this.onClickofTumbnail}
                  />
                ))}
              </ul>
            </div>
